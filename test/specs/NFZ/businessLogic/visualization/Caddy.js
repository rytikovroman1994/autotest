describe.skip('test screenshots cars', () => {
    ...
    // проверяем доступность коплектаций в каточках
        it(`Make a screenshot California`, () => {
            ...
            if(browser.isVisible(`.avn008_model__wrap > div.gridcontainer > div:nth-child(1)`) === false) {
                browser.scroll(`.avn008_model__wrap > div.gridcontainer > div:nth-child(1)`, 5, 5);
            }
            // получаем имя автомобиля
            nameCar = browser.getText(`div:nth-child(1) > div > div > div.avn008_car__info > div.avn008_car__desc > h4`);
            console.log(nameCar);
            browser.scroll(`.avn008_model__wrap > div.gridcontainer > div:nth-child(1)`, 0, 0);
            // кликаем на карточку
            browser.click(`.avn008_model__wrap > div.gridcontainer > div:nth-child(1)`);
            ...
            for( let y = 1; y <= numberEquipment.length - 1; y++ ) {
                ...
                var screen = browser.saveScreenshot(`./screenshotModel/versionThree/size1400x1050/${nameCar}-${nameEquipment}.png`);
